...
def collate(batch, samples_per_gpu=1, collate_fn=default_collate):
    ...
    if isinstance(batch[0], DataContainer):
        ...
        return [collate(samples, samples_per_gpu, collate_fn) for samples in transposed]
    elif isinstance(batch[0], Mapping):
        return {key: collate([d[key] for d in batch], samples_per_gpu, collate_fn) for key in batch[0]}
    else:
